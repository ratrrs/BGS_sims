...
read_sim<-function(pidata,xidata,tajdata,mu=1.66e-8,neutral=FALSE,windows=101){
  ...
  data<-rbind(xi,pi,tajD) %>%
    ...
    mutate(mean_val=ifelse(neutral==FALSE & centimorgen==0,mean_val*1.25,mean_val))
  return(data)
}
#neutral=TRUE
...
plot_ratios <- function(data,events,windows=51){
  center <- windows/2+.5
...
```{r}
plot_over_time <- function(data,events,stats,windows=51){
  center <- windows/2+.5
  window_list <- c(center,center-10,center,center+10,center-20,center+20,1,windows,center+5,center-5)
  comb <- data %>%
