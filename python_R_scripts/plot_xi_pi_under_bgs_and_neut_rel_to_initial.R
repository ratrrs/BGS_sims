...
if(models == '1012'){
	pdf('model2to4_0.00082cM_rel_to_initial.pdf', height=6, width=7.5)
	par(mfrow=c(3,3), oma=c(5.5,8.7,0,0) + 0.1, mar=c(0,0,0.9,0.9), las=1)
} else if(models == '25'){
	pdf('model5to8_0.00082cM_rel_to_initial.pdf', height=6, width=9.5)
	par(mfrow=c(3,4), oma=c(5.5,8.55,0,0) + 0.1, mar=c(0,0,0.9,0.9), las=1)
} else if(models == '69'){
	pdf('model9to12_0.00082cM_rel_to_initial.pdf', height=6, width=9.5)
	par(mfrow=c(3,4), oma=c(5.5,8.55,0,0) + 0.1, mar=c(0,0,0.9,0.9), las=1)
}
...
#####################################
###start plot raw xi and pi values###
#####################################

#if plotting xi and pi for models 2-8, need all generations from data file
#if plotting xi and pi for models 9-12, only need last 41 generations from data file
if(models == '25' | models == '1012'){
	start = 5 #start at 5 to avoid burn-in generations
} else{
	start = 365 #start at 365 to avoid burn-in generations
}	

#make function to plot pi, pi0, xi, and xi0 (relative to the initial/first generation) for each generation in the simulation model 
plot_func = function(meanfile_neut, CIlowerfile_neut, CIupperfile_neut, meanfile_bgs, CIlowerfile_bgs, CIupperfile_bgs, YMIN, YMAX, BY, NUMDIGITS, LWD, lowerlabel=FALSE){
	...
	yseq=c(seq(YMIN, YMAX, BY))
	ylabs=as.character(round(yseq, digits=NUMDIGITS))
	yseq=as.numeric(ylabs) #update since we just rounded the positions

	if(YMAX == 1.0){
		ylabs[length(ylabs)] = '1.0'
	}
	
	#find the value closest to 1 in the vector of y-axis label positions
	...
	if(lowerlabel == FALSE){
		...
		ylabs[which(myvec == min(myvec))] = '1.0'
		yseq[which(myvec == min(myvec))] = 1.0
	} else{
		if(models == '1012'){
			myvec = c()
			for(i in yseq){
			...
			ylabs[length(ylabs)] = '1.0'
			yseq[length(yseq)] = 1.0				
		}
	}
	
	if(ylabs[1] == "0.0e+00" | ylabs[1] == '0'){
		ylabs[1] = '0.0'
	}
	...
	subplot_CI = function(YCONST, X, YDEM1, YDEM2, YCONST_CIlower, YCONST_CIupper, YDEM1_CIlower, YDEM1_CIupper, YDEM2_CIlower, YDEM2_CIupper, XLABEL=lowerlabel, YLABEL=FALSE){
		...
		if(YLABEL==TRUE){
			...
			if(lowerlabel == FALSE){ #plotting row of singleton (xi) plots so add corresponding yaxis title
			 	text(par("usr")[1]-(0.535*(par("usr")[2]-par("usr")[1])), (YMIN+YMAX)/2, expression(paste('Singleton Density (', xi, ')', sep='')), srt=90, xpd=NA, cex=1.4)
			 	text(par("usr")[1]-(0.445*(par("usr")[2]-par("usr")[1])), (YMIN+YMAX)/2, 'Compared to Initial', srt=90, xpd=NA, cex=1.4)
			} else{ #plotting row of diversity (pi) plots so add corresponding yaxis title
			 	text(par("usr")[1]-(0.535*(par("usr")[2]-par("usr")[1])), (YMIN+YMAX)/2, expression(paste('Diversity (', pi, ')', sep='')), srt=90, xpd=NA, cex=1.4)
			 	text(par("usr")[1]-(0.445*(par("usr")[2]-par("usr")[1])), (YMIN+YMAX)/2, 'Compared to Initial', srt=90, xpd=NA, cex=1.4)
			 }
		}
		...
		if(XLABEL == TRUE){
			...
			if(models == '25' | models == '1012'){ #models 2-8
				...
				text(22.25, par("usr")[3] - 1.8*par("usr")[3],  expression(paste('Time (', italic('N'[anc]), ' generations)', sep='')), xpd=NA, cex=1.4)
				text(xpos, par("usr")[3] - 0.47*par("usr")[3], labels=xlab, srt=45, adj=1, pos=NULL, xpd=NA, cex=1.25)
			}
	...
	#make new dataframes with all points relative to the first index (i.e., first simulated generation of demography post- burn-in)
	df_mean_new_bgs = data.frame()
	df_lower_new_bgs = data.frame()
	df_upper_new_bgs = data.frame()
	for(z in 1:nrow(df_mean_bgs)){
		if(length(x) == 42){
			newmean_vec = c(1) #add an extra "null" generation so plots start a little before first simulated generation
			newlower_vec = c(1) #add an extra "null" generation so plots start a little before first simulated generation
			newupper_vec = c(1) #add an extra "null" generation so plots start a little before first simulated generation
		} else{
			newmean_vec = c(1, 1, 1) #add extra "null" generations so plots start a little before first simulated generation
			newlower_vec = c(1, 1, 1) #add extra "null" generations so plots start a little before first simulated generation
			newupper_vec = c(1, 1, 1) #add extra "null" generations so plots start a little before first simulated generation		
		}
		for(k in 1:ncol(df_mean_bgs)){
			newmean_vec = c(newmean_vec, df_mean_bgs[z,k]/df_mean_bgs[z,1])
			newlower_vec = c(newlower_vec, df_lower_bgs[z,k]/df_lower_bgs[z,1])
			newupper_vec = c(newupper_vec, df_upper_bgs[z,k]/df_upper_bgs[z,1])
			}
		df_mean_new_bgs = rbind(df_mean_new_bgs, newmean_vec)
		df_lower_new_bgs = rbind(df_lower_new_bgs, newlower_vec)
		df_upper_new_bgs = rbind(df_upper_new_bgs, newupper_vec)
	}
	df_mean_bgs = df_mean_new_bgs
	df_lower_bgs = df_lower_new_bgs
	df_upper_bgs = df_upper_new_bgs

	df_mean_new_neut = data.frame()
	df_lower_new_neut = data.frame()
	df_upper_new_neut = data.frame()
	for(z in 1:nrow(df_mean_neut)){
		if(length(x) == 42){
			newmean_vec = c(1) #add an extra "null" generation so plots start a little before first simulated generation
			newlower_vec = c(1) #add an extra "null" generation so plots start a little before first simulated generation
			newupper_vec = c(1) #add an extra "null" generation so plots start a little before first simulated generation
		...
		for(k in 1:ncol(df_mean_neut)){
			newmean_vec = c(newmean_vec, df_mean_neut[z,k]/df_mean_neut[z,1])
			newlower_vec = c(newlower_vec, df_lower_neut[z,k]/df_lower_neut[z,1])
			newupper_vec = c(newupper_vec, df_upper_neut[z,k]/df_upper_neut[z,1])
			}
		df_mean_new_neut = rbind(df_mean_new_neut, newmean_vec)
		df_lower_new_neut = rbind(df_lower_new_neut, newlower_vec)
		df_upper_new_neut = rbind(df_upper_new_neut, newupper_vec)
	}
	df_mean_neut = df_mean_new_neut
	df_lower_neut = df_lower_new_neut
	df_upper_neut = df_upper_new_neut
	
	#make vectors for pi and pi0 (or xi and xi0) values (relative to the initial/first generation) and their lower and upper CIs
	...
	#plot models pi and pi0 (or xi and xi0)	(relative to the initial/first generation)
...
#set spacing for positions for y-axis tick marks/grid lines
numdigits = 2
by = round((ymax-ymin)/5, digits=numdigits)

plot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, ymin, ymax, by, numdigits, w, FALSE)
...
plot_func(mean_neut_pi_file, lower_neut_pi_file, upper_neut_pi_file, mean_bgs_pi_file, lower_bgs_pi_file, upper_bgs_pi_file, ymin, ymax, by, numdigits, w, TRUE)

################
##end plot pis##
################

#make a subplot function to plot insets for pi, pi0, xi, and xi0  (relative to the initial/first generation) for each generation in the simulation model with truncated x- and y-axes
subplot_func = function(meanfile_neut, CIlowerfile_neut, CIupperfile_neut, meanfile_bgs, CIlowerfile_bgs, CIupperfile_bgs, XMIN, XMAX, YMIN, YMAX, LWD, cols, LOG){
	...
	if(models == '25' | models == '1012'){
		...
		x = c(0.8, 2, 3.2, 4, 4.8, 6, 7.2, 8, 8.8, 10, 11.2, 12, 12.8, 14, 15.2, 16, 16.8, 18, 19.2, 20, 20.8, 22, 23.2, 24, 24.8, 26, 27.2, 28, 28.8, 30, 31.2, 32, 32.8, 34, 35.2, 36, 36.8, 38, 39.2, 40, 40.8, 42, 43.2, 44, 44.8, 46, 47.2, 48, 48.8, 50, 51.2, 52, 52.8, 54, 55.2, 56, 56.8, 58, 59.2, 60, 60.8, 62, 63.2, 64, 64.8, 66, 67.2, 68, 68.8, 70, 71.2, 72, 72.8, 74, 75.2, 76, 76.8, 78, 79.2, 80, 80.8, 82, 83.2, 84, 84.8, 86, 87.2, 88, 88.8, 90, 91.2, 92, 92.8, 94, 95.2, 96, 96.8, 98, 99.2, 100, 100.8, 102, 103.2, 104, 104.8, 106, 107.2, 108, 108.8, 110, 111.2, 112, 112.8, 114, 115.2, 116, 116.8, 118, 119.2, 120, 120.8, 122, 123.2, 124, 124.8, 126, 127.2, 128, 128.8, 130, 131.2, 132, 132.8, 134, 135.2, 136, 136.8, 138, 139.2, 140, 140.8, 142, 143.2, 144, 144.8, 146, 147.2, 148, 148.8, 150, 151.2, 152, 152.8, 154, 155.2, 156, 156.8, 158, 159.2, 160, 160.8, 162, 163.2, 164, 164.8, 166, 167.2, 168, 168.8, 170, 171.2, 172, 172.8, 174, 175.2, 176, 176.8, 178, 179.2, 180, 180.8, 182, 183.2, 184, 184.8, 186, 187.2, 188, 188.8, 190, 191.2, 192, 192.8, 194, 195.2, 196, 196.8, 198, 199.2, 200, 200.8, 202, 203.2, 204, 204.8, 206, 206.8, 208, 208.8, 210, 211.2, 212, 212.8, 214, 215.2, 216, 216.8, 218, 219.2, 220, 220.8, 222, 223.2, 224, 224.8, 226, 227.2, 228, 228.8, 230, 231.2, 232, 232.8, 234, 235.2, 236, 236.8, 238, 239.2, 240, 240.8, 242, 243.2, 244, 244.8, 246, 247.2, 248, 248.8, 250, 251.2, 252, 252.8, 254, 255.2, 256, 256.8, 258, 259.2, 260, 260.8, 262, 263.2, 264, 264.8, 266, 267.2, 268, 268.8, 270, 271.2, 272, 272.8, 274, 275.2, 276, 276.8, 278, 279.2, 280, 280.8, 282, 283.2, 284, 284.8, 286, 287.2, 288, 288.8, 290, 291.2, 292, 292.8, 294, 295.2, 296, 296.8, 298, 299.2, 300, 300.8, 302, 303.2, 304, 304.8, 306, 307.2, 308, 308.8, 310, 311.2, 312, 312.8, 314, 315.2, 316, 316.8, 318, 319.2, 320, 320.8, 322, 323.2, 324, 324.8, 326, 327.2, 328, 328.8, 330, 331.2, 332, 332.8, 334, 335.2, 336, 336.8, 338, 339.2, 340, 340.8, 342, 343.2, 344, 344.8, 346, 347.2, 348, 348.8, 350, 351.2, 352, 352.8, 354, 355.2, 356, 356.8, 358, 359.2, 360, 360.8, 362, 363.2, 364, 364.8, 366, 367.2, 368, 368.8, 370, 371.2, 372, 372.8, 374, 375.2, 376, 376.8, 378, 379.2, 380, 380.8, 382, 383.2, 384, 384.8, 386, 387.2, 388, 388.8, 390, 391.2, 392, 392.8, 394, 395.2, 396, 396.8, 398, 399.2, 400, 400.8, 402, 403.2, 404)
		#make vector of positions for placing x-axis labels
		xpos = seq(4, 404, 20)[c(1, 2, 3)]
		#x-axis labels
		xlab = c('-1.0', '-0.95', '-0.9')
		#alternative spacing between labels
		if(XMAX > 100){
			if(models == '1012'){
				xpos = seq(4, 404, 40)[c(1, 2, 3)]
				xlab = c('-1.0', '-0.9', '-0.8')				
			} else{
				xpos = seq(4, 404, 20)[c(1, 2, 3, 4, 5, 6, 7)]
				xlab = c('-1.0', '', '-0.9', '', '-0.8', '', '-0.7')
			}
		}
	} else{
		#make vector for positions along x-axis for plotting pi/pi0 and xi/xi0 for each generation (42 total for models 9-12)
		x = c(1.5, 2, 2.8, 4, 5.2, 6, 6.8, 8, 9.2, 10, 10.8, 12, 13.2, 14, 14.8, 16, 17.2, 18, 18.8, 20, 21.2, 22, 22.8, 24, 25.2, 26, 26.8, 28, 29.2, 30, 30.8, 32, 33.2, 34, 34.8, 36, 37.2, 38, 38.8, 40, 41.2, 42)
		#make vector of positions for placing x-axis labels
		xpos = seq(2, 42, 8)[seq(1, 3, 1)]
		#x-axis labels
		xlab = c('-0.1', '-0.09', '-0.08')
	}
	...
	#relabel y-axis labels from rounding
	ylabs = formatC(as.numeric(ylabs), format = "e", digits = 1)
	ylabs = formatC(as.numeric(ylabs), digits = 4)
	...
	#make inset plots for models' pi and pi0 (or xi and xi0) (relative to the initial/first generation) if they are in the plot column where cols == 'column number'
	if(cols=='1'){
...
if(models != '1012'){
	if(models == '25'){
		...
		par(fig=c(0.06+.01, 0.215-.02, 0.49, 0.652), new=T) #coordinates for inset
		subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, 50, ymin1, ymax1, w, '1', logoption)
		
		#inset plot for row 2, column 3 (xi for generic model 7)	
		par(fig=c(0.56+.01, 0.715-.02, 0.49, 0.652), new=T) #coordinates for inset
		subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, 50, ymin3, ymax3, w, '3', logoption)
		
		#inset plot for row 3, column 2 (pi for generic model 6)	
		par(fig=c(0.31+.01, 0.465-.01, 0.49-.335, 0.652-.335), new=T) #coordinates for inset
		subplot_func(mean_neut_pi_file, lower_neut_pi_file, upper_neut_pi_file, mean_bgs_pi_file, lower_bgs_pi_file, upper_bgs_pi_file, xmin, 120, 0.08, 0.5, w, '2', logoption)
		
		#inset plot for row 3, column 4 (pi for generic model 8)			
		par(fig=c(0.81+.01, 0.965-.01, 0.49-.335, 0.652-.335), new=T) #coordinates for inset
		subplot_func(mean_neut_pi_file, lower_neut_pi_file, upper_neut_pi_file, mean_bgs_pi_file, lower_bgs_pi_file, upper_bgs_pi_file, xmin, 120, 0.06, 0.3, w, '4', logoption)
	} else{
		#inset plot for row 2, column 1 (xi for generic model 9)
		par(fig=c(0.06+.01, 0.215-.03, 0.49, 0.652), new=T) #coordinates for inset
		subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, 16.7, ymin1, ymax1, w, '1', logoption)
		
		#inset plot for row 2, column 3 (xi for generic model 11)
		par(fig=c(0.56+.01, 0.715-.01, 0.49, 0.652), new=T) #coordinates for inset
		subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, xmax, ymin3, ymax3, w, '3', logoption)
	}
	...
	subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, xmax, ymin2, ymax2, w, '2', logoption)
	...
	subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, xmax, ymin4, ymax4, w, '4', logoption)
} else{
	#inset plot for row 2, column 1 (xi for generic model 2)
	par(fig=c(0.117, 0.292, 0.49, 0.652), new=T) #coordinates for inset
	subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, 105, ymin1, ymax1, w, '1', logoption)
	
	#inset plot for row 2, column 2 (xi for generic model 3)
	par(fig=c(0.45, 0.625, 0.49, 0.652), new=T) #coordinates for inset
	subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, 105, ymin2, ymax2, w, '2', logoption)
	
	#inset plot for row 2, column 3 (xi for generic model 4)
	par(fig=c(0.784, 0.959, 0.49-0.075, 0.652-0.075), new=T) #coordinates for inset
	subplot_func(mean_neut_xi_file, lower_neut_xi_file, upper_neut_xi_file, mean_bgs_xi_file, lower_bgs_xi_file, upper_bgs_xi_file, xmin, 105, ymin3, ymax3, w, '3', logoption)
	
	#inset plot for row 3, column 2 (pi for generic model 3)
	par(fig=c(0.45, 0.625, 0.49-.3325, 0.652-.3325), new=T) #coordinates for inset
	subplot_func(mean_neut_pi_file, lower_neut_pi_file, upper_neut_pi_file, mean_bgs_pi_file, lower_bgs_pi_file, upper_bgs_pi_file, xmin, 105, ymin4, ymax4, w, '2', logoption)
}
