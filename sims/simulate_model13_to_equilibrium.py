...
mu = 1.66e-8
Nend= int(demog.item(-1))

demog = np.concatenate((demog,np.array([Nend]*int(10*Nstart),dtype=np.uint32))).astype(np.int32)
...
burnin_name = out_path + "../generic2/burnins/burnin_neut_%s.lzma9" % replicate
...
with lzma.open(burnin_name, 'rb') as f:
    pop2 = pickle.load(f)
print(mypop==pop2)
print("Generation after burnin load",pop2.generation)
...
#ppop = pickle.dumps(mypop,-1)
# pickle equilibirum population
burnin_name = out_path + "../generic2/burnins/burnin_bgs_%s.lzma9" % replicate
#with lzma.open(burnin_name, "wb", preset=9) as f:
#    pickle.dump(mypop, f, -1)
